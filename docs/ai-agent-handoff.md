...
- 로그인 후 PC 헤더에는 사용자명과 로그인 중 상태가 표시되고, 태블릿·모바일은
  사용자 아이콘의 녹색 점과 모바일 메뉴의 사용자 정보·로그아웃으로 상태를 구분한다.
- 관리자·편집자 모바일 메뉴에는 공개 메뉴 바로 아래 보라색 `관리자 화면` 링크가
  표시되며 우측 로그인 상태 아이콘도 관리자 화면으로 이동한다.
