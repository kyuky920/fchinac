...
마이그레이션 적용 순서는 루트 `README.md`를 따른다. 현재 최신 번호는 `014`다.
...
5. CI가 현재 `001`, `003`~`005`만 적용하는 문제 수정: `006`~`014`와 `009` 강의
   스키마를 새 DB에서 검증하도록 워크플로를 갱신해야 한다.
6. 회원 본인 프로필·비밀번호 변경
7. 이메일 자동 발송과 비밀번호 재설정 전달 자동화
8. 번역 검색·완료율·누락 필터 및 실제 15개 언어 번역 확충
9. 법률 전문가의 이용약관·개인정보 처리방침 최종 검토
10. 회원별 강의 진도·재생 이력과 개인별 다운로드 이력

10번은 사용자가 추후 진행하기로 한 기능이다. 현재는 첨부파일별 총
